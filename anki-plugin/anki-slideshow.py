...
EXAMPLE_SYNC_TARGET = 'talonweb9@192.168.4.5:/var/www/cards.reedandmegan.net/web/anki-data'
SYNC_CMD = ['c:/cygwin64/bin/bash.exe -c "cd /cygdrive/c/Users/Reed/Documents/Anki/C-17/; rsync -rcz --stats']
...
def getPaths():
	...
	json_file = 'anki-slideshow.json'
	json_filename = os.path.join(db_path, json_file)
	media_path = os.path.join(db_path, 'collection.media')
	return json_file, json_filename, media_path

def getPrevData():
    json_file, json_filename, media_path = getPaths()
    try:
        prev_data = json.load(io.open(json_filename, 'r'))
    except IOError:
        prev_data = {}
    return prev_data
...
def exportCardsToWeb():
    ...
    if export['sync_target'] != '':
        try:
            args = SYNC_CMD + [json_file, 'collection.media', export['sync_target']]
